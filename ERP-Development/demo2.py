        ...
        with open(INDENT_CSV_FILE, 'r') as file:
            ...
            for row in reader:
                ...
                if not indent_number:
                    continue  # Skip if indent_number is empty
                ...
                try:
                    ...
                    # Skip rows with invalid or incorrectly formatted indent numbers
                    continue
...
@app.route('/download_indent', methods=['GET', 'POST'])
def download_indent():
    indent_number = request.form.get('indent_number') if request.method == 'POST' else request.args.get('indent_number')
    indent = get_indent_by_number(indent_number)
...
def send_indent_email(indent):
    recipients = ['shelardipesh18@gmail.com', 'pratik.bhirud@defspace.com']
    ...
    # Generate the URL for downloading the indent details
    download_indent_url = url_for('download_indent', indent_number=indent['indent_number'], _external=True)
    
    # Email body without items
    body = f"""
    ...
    Download Indent: {download_indent_url}
    """
...
def update_po_in_csv(po_number, updated_rows):
    """Update an existing purchase order in the CSV file."""
    all_rows = []
    ...
    # Add the updated rows for the given PO number
...
def purchaseorder(indent_number):
    ...
    if po_number:
        ...
        # If no po_number is provided, generate a new one
        po_number = generate_po_number()

    if request.method == 'POST':
        # Handle form submission
        po_number = request.form['po_number']  # Get PO number from the form
        ...
        # Update or insert the PO in the CSV file
        existing_po = get_po_by_number(po_number)
        if existing_po:
            # Update existing PO
            update_po_in_csv(po_number, updated_rows)
        else:
            # Add new PO
        ...
        download_po_url = url_for('download_po', po_number=po_number, _external=True)
        # Send email
...
@app.route('/download_po', methods=['GET'])
def download_po():
    po_number = request.args.get('po_number')
    ...
    po_list = get_po_by_number(po_number)
    ...
    if po_list:
        ...
        return render_template('download_po.html',
...
def save_pa():
    ...
    pa_url = url_for('create_pa', pa_number=new_pa['PA No.'], _external=True)
    download_pa_url = url_for('download_pa', pa_number=pa['PA No.'], _external=True)
    # Send email with PA details
    msg = Message('Payment Advice Created/Updated', sender='shelardipesh18@gmail.com', recipients=['shelardipesh18@gmail.com'])
    msg.body = f"""
    PA Number: {new_pa['PA No.']}
    PO Number: {new_pa['PO No.']}

    Prepared By: {new_pa['Prepared By']}
    Initiated By: {new_pa['Initiated By']}
    Checked By: {new_pa['Checked By']}
    Authorised By: {new_pa['Authorised By']}
    Approved By: {new_pa['Approved By']}
    
    Direct PA Link: {pa_url}
    Server Link: http://127.0.0.1:5000/
    download PA :{download_pa_url}
    """
    mail.send(msg)
...
@app.route('/download_pa', methods=['GET', 'POST'])
def download_pa():
    if request.method == 'POST':
        pa_number = request.form.get('pa_number')
    else:
        pa_number = request.args.get('pa_number')

    if not pa_number:
        return "PA Number is required!", 400
    ...
    if pa_list:
        ...
        return render_template('paformat.html',
                                   pa_number=pa['PA No.'],
                                   date=pa['Date'],
...
def generate_grn_number():
    """Generate a new GRN number based on the highest existing GRN number in the CSV."""
    highest_grn_number = 0
    
    try:
        with open(GRN_FILE, 'r') as file:
            reader = csv.reader(file)
            next(reader)  # Skip the header row if it exists
            
            for row in reader:
                grn_number = row[0]  # Assuming GRN number is in the first column
                # print(f"Processing GRN number: {grn_number}")
                
                # Extract the numeric part of the GRN number
                try:
                    number_part = int(grn_number.split('-')[-1])
                    # print(f"Extracted numeric part: {number_part}")
                    
                    if number_part > highest_grn_number:
                        highest_grn_number = number_part
                except (ValueError, IndexError):
                    print(f"Skipping invalid GRN number: {grn_number}")
                    continue  # Skip invalid or incorrectly formatted GRN numbers
    
    except FileNotFoundError:
        print(f"{GRN_FILE} not found, starting from 1.")
        pass  # If the file doesn't exist, start from 1
    
    # Generate the next GRN number
    next_grn_number = highest_grn_number + 1
    # print(f"Next GRN number to be generated: {next_grn_number}")
    
    return f'PADTPL-GRN-24-25-{next_grn_number:04d}'

# Example usage:
new_grn_number = generate_grn_number()
# print(f"Generated GRN number: {new_grn_number}")
...
def add_grn():
    po_number = request.args.get('po_number')
    vendor_name = None  # Initialize vendor_name
    if request.method == 'POST':
        ...
        with open(PURCHASE_ORDERS_FILE, 'r') as file:
            ...
            for row in reader:
                if row['po_number'] == po_number:
                    ...
                    vendor_name = row['Vendor']  # Assume the vendor name is stored in a column named 'Vendor'
    ...
    return render_template('add_grn.html', po_number=po_number, vendors=vendors, grn_number=generate_grn_number(), items=items,vendor_name=vendor_name)
...
def get_grn_by_number(grn_number):
    ...
    with open(GRN_FILE, mode='r') as file:
        reader = csv.DictReader(file)
        for row in reader:
            # Ensure GRN Number comparison is consistent
...
def download_grn(encoded_grn_number):
    ...
    if grn_details:
        ...
        return render_template('download_grn.html',
